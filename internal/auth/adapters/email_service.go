...
const SendVerificationEmailBody = `Olá,

Bem-vindo ao Momento! Por favor, confirme seu email clicando no link abaixo:

%s

Este link é válido por 24 horas.

Se você não criou uma conta no Momento, ignore este email.
...
func NewEmailService(host, user, pass, from, resetURLBase, verificationURL, port string) *emailService {
	return &emailService{
		host:            host,
		port:            port,
		user:            user,
...
func (s *emailService) SendVerificationEmail(ctx context.Context, to, token string) error {
	...
	subject := "Confirmação de Email - Momento"
	body := fmt.Sprintf(SendVerificationEmailBody, verificationLink)
	msg := fmt.Sprintf("From: %s\r\nTo: %s\r\nSubject: %s\r\n\r\n%s", s.from, to, subject, body)
	addr := fmt.Sprintf("%s:%s", s.host, s.port)
	var auth smtp.Auth
	if s.user != "" && s.pass != "" {
